...
1. Envelope method as primary
2. IQR outlier rejection
3. Velocity sanity checks
4. GCC-PHAT refinement only when it helps
...
def extract_dispersion(signals, dt, distances, frequencies, verbose=False):
    """
    Extract dispersion using envelope method with optional GCC-PHAT refinement.
    Always returns results using envelope as fallback.
    """
    ...
    for f_center in frequencies:
        pair_velocities = []
        pair_velocities_refined = []  # For GCC-PHAT refined values
        ...
        for i in range(n_receivers - 1):
            for j in range(i + 1, n_receivers):
                ...
                # Envelope-based delay (always calculate)
                ...
                # Compute envelope velocity
                if coarse_delay > 1e-9:
                    v_envelope = distance / coarse_delay
                    if 0.5 <= v_envelope <= 10:
                        pair_velocities.append(v_envelope)
                
                # Try GCC-PHAT refinement
                try:
                    ...
                    # Validate refinement
                    if abs(delay_refined - coarse_delay) < period / 2 and delay_refined > 1e-9:
                        v_refined = distance / delay_refined
                        if 0.8 <= v_refined <= 2.5:
                            pair_velocities_refined.append(v_refined)
                except:
                    pass  # GCC-PHAT failed, use envelope
        
        # Choose best set of velocities
        if pair_velocities_refined and len(pair_velocities_refined) >= 2:
            # Use GCC-PHAT refined velocities
            v_array = np.array(pair_velocities_refined)
            method_used = 'gcc_phat'
        elif pair_velocities:
            # Fall back to envelope velocities
            v_array = np.array(pair_velocities)
            method_used = 'envelope'
        else:
            continue  # No valid measurements for this frequency
        ...
        if 0.8 <= v_mean <= 2.5:
            ...
            results['methods'].append(method_used)
            
            if verbose:
                print(f"  {f_center:.0f} Hz: c = {v_mean:.2f} ± {v_std:.2f} m/s ({method_used})")
...
def run_pipeline(true_G=2000, true_eta=0.5, rho=1000, noise_level=0.10):
    ...
    print("="*70)
    print("END-TO-END PIPELINE - WORKING VERSION")
    print("="*70)
    ...
    # Extract dispersion
    print("\n2. Extracting dispersion...")
    dispersion = extract_dispersion(signals, dt, distances, frequencies, verbose=True)
...
Goodness of fit:
  R² = {fit['r_squared']:.4f}
    ...
    plt.suptitle('End-to-End Pipeline (Working)', fontweight='bold', fontsize=14)
    plt.tight_layout()
    plt.savefig('e2e_pipeline_working.png', dpi=150, bbox_inches='tight')
    print("   Saved: e2e_pipeline_working.png")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='End-to-End Pipeline (Working)')
