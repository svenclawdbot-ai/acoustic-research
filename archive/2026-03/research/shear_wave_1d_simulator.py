...
class ShearWave1D:
    ...
    def step(self):
        ...
        For Kelvin-Voigt model, the update is:
        u_next = 2*u - u_prev + (c_s*dt/dx)² * (u[i+1] - 2*u[i] + u[i-1])
                  + (eta*dt/rho*dx²) * (v[i+1] - 2*v[i] + v[i-1])
        
        Where v = (u - u_prev)/dt is velocity.
        """
        ...
        # Update equation (Kelvin-Voigt viscoelasticity)
